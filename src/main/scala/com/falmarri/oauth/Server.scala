...
import com.falmarri.jwt.{JWTClient, ClientCredentials}
...
object SearchServer{
  ...
  def main(args: Array[String]) {
    ...
    val client = Try {
      JWTClient(ClientCredentials(p.key, p.client_id), p.host)
    }  match {
      case Success(v) =>
        v
      case Failure(e) =>
...
object SearchService {
  ...
  def apply(searchType: String, query: String, client: JWTClient) = {
...
case class Search(client: JWTClient, query: Option[String], fields: Seq[String] = Seq())
